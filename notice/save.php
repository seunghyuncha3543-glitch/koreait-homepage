...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') { fail('공고 저장은 관리자 페이지에서 작성 후 저장해야 합니다.', 405); }
...
if ($hasFile) {
  ...
  $savedName = date('YmdHis') . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
  if (!move_uploaded_file($_FILES['attachment']['tmp_name'], $uploadDir . '/' . $savedName)) { fail('첨부파일 저장에 실패했습니다. uploads 폴더 권한을 확인해 주세요.'); }
  $attachment = ['original_name' => $originalName, 'saved_name' => $savedName, 'path' => 'uploads/' . $savedName];
}

$dataFile = $dataDir . '/notices.json';
if (!file_exists($dataFile)) { file_put_contents($dataFile, json_encode([], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)); }
$notices = json_decode(file_get_contents($dataFile), true);
if (!is_array($notices)) { $notices = []; }

$notices[] = ['id' => date('YmdHis') . '-' . bin2hex(random_bytes(3)), 'title' => $title, 'content' => $content, 'created_at' => date('Y-m-d H:i:s'), 'attachment' => $attachment];
